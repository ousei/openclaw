  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes.cardTitle")}</div>
          <div class="card-sub">${t("nodes.cardSub")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.noNodesFound")}</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodes.devicesSub")}</div>
        </div>
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodes.execNodeBinding")}</div>
          <div class="card-sub">
            ${t("nodes.execNodeBindingSub")}
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("cron.saving") : t("agents.save")}
        </button>
      </div>
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("config.loadConfigToEdit")}</div>
          ...
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-sub">${t("nodes.defaultBindingSub")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>Node</span>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t("nodes.defaultAgent") : t("nodes.agent")} ·
          ${
            bindingValue === "__default__"
              ? `${t("nodes.usesDefault")} (${state.defaultBinding ?? t("nodes.anyNode")})`
              : `${t("nodes.override")}: ${agent.binding}`
          }
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes.binding")}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
