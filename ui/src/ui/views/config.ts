...
function getSections(): Array<{ key: string; label: string }> {
  return SECTION_KEYS.map((key) => ({ key, label: t(`config.sections.${key}`) }));
}
...
export function renderConfig(props: ConfigProps) {
  ...
  const extraSections = Object.keys(schemaProps)
    ...
    .map((k) => {
      const translated = t(`config.sections.${k}`);
      return { key: k, label: translated === `config.sections.${k}` ? humanize(k) : translated };
    });
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          ...
          <span
            ...
            >${validity === "valid" ? t("common.valid") : validity === "invalid" ? t("common.invalid") : t("common.unknown")}</span
          >
        </div>
        ...
        <div class="config-actions">
                  ...
                  <span class="config-changes-badge"
                    >${
                      props.formMode === "raw"
                        ...
                        : diff.length === 1
                          ? t("config.unsavedChange")
                          : t("config.unsavedChangeCount", { count: String(diff.length) })
                    }</span
                  >
                `
                : html`
          ...
          <div class="config-actions__right">
            ...
            <button
              ...
              @click=${props.onSave}
            >
              ${props.saving ? t("cron.saving") : t("agents.save")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
