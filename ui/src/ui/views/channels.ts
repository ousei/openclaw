...
${props.snapshot ? JSON.stringify(props.snapshot, null, 2) : t("channels.noSnapshotYet")}
      </pre>
    </section>
  `;
}
...
function formatStatus(s: "yes" | "no" | "active" | "na"): string {
  return s === "yes" ? t("common.yes") : s === "no" ? t("common.no") : s === "active" ? t("common.active") : t("common.na");
}
  ...
  return html`
    <div class="account-card">
      ...
      <div class="status-list account-card-status">
        <div>
          <span class="label">${t("channels.running")}</span>
          <span>${formatStatus(runningStatus)}</span>
        </div>
        <div>
          <span class="label">${t("channels.configured")}</span>
        ...
        <div>
          <span class="label">${t("channels.connected")}</span>
          <span>${formatStatus(connectedStatus)}</span>
        </div>
        <div>
