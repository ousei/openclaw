      ...
      ${
        subsectionContext
          ...
          : filteredEntries.map(([key, node]) => {
              const meta = SECTION_META[key] ?? { label: key.charAt(0).toUpperCase() + key.slice(1), description: node.description ?? "" };
              const metaLabel = t(`config.sections.${key}`) !== `config.sections.${key}` ? t(`config.sections.${key}`) : meta.label;
              ...
              <section class="config-section-card" id="config-section-${key}">
                <div class="config-section-card__header">
                  ...
                  <div class="config-section-card__titles">
                    <h3 class="config-section-card__title">${metaLabel}</h3>
                    ${
                      meta.description
